      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            <div style="font-size:.75rem;color:var(--mid);">Upload: <?= h($r['created_at']) ?></div>
          </td>
          ...
          <td>
            <?php if ($r['status'] === 'signed'): ?>
              <a href="?page=download-signed&id=<?= $r['id'] ?>" class="btn btn-teal" style="padding:.3rem .8rem;font-size:.8rem;">⬇ Unduh</a>
            <?php elseif ($r['status'] === 'uploaded'): ?>
              <a href="?page=sign-document&doc=<?= $r['id'] ?>" class="btn btn-primary" style="padding:.3rem .8rem;font-size:.8rem;">🖊 Tanda Tangani</a>
            <?php endif; ?>
          </td>
        </tr>
        <?php endforeach; ?>
